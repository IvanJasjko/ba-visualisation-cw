...
Filtering dataset to obtain relevant columns that are used for for our analysis. Grouping data to get mean reveiw scores across categories.
```{r}
review_cols = c("price_level","kpi_score","review_scores_cleanliness",
                      "kpi_score","review_scores_communication","review_scores_location",
                      "review_scores_value"
                      )
...
review_data_sum <- review_data %>% group_by(price_level, kpi_score) %>% 
  summarise(
    cleanliness = mean(na.omit(review_scores_cleanliness)),
    communication = mean(na.omit(review_scores_communication)),
    location = mean(na.omit(review_scores_location)),
    value = mean(na.omit(review_scores_value))
  )
```
# Plotting distribution of reviews across price levels and occupancy rates
```{r}
rd <- review_data %>% 
  pivot_longer(cols=c("review_scores_cleanliness", "review_scores_communication", "review_scores_location", "review_scores_value"),names_to="type",values_to="score") %>% unite(kpi_type, c("kpi_score","type"))

ggplot(rd, aes(y=kpi_type, x=score,  fill=kpi_type)) +
...
# Reviews Lineup
Comparison of mean review scores across price and occupancy levels.
```{r}
lineup(review_data_sum)
```
