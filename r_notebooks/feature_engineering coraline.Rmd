...
```{r}
requirements <- c("ggplot2", "dplyr", "stringr", "tidyr", "ggrepel", "splitstackshape", "stringr","RColorBrewer","OIdata")
...
### Looking at postings info
```{r}
#"name", "instant_bookable", "description", "neighborhood_overview", "minimum_nights",
#"maximum_nights", "availability_30", "accommodates"
...
description_analysis=description_analysis %>%
  ...
  summarise(descrip = p(description_cleansed))
description_analysis$descrip=str_replace_all(description_analysis$descrip, "[^[:alnum:]]", "")
description_analysis$descrip=str_split(description_analysis$descrip, boundary("word"))
```

```{r}
#summary, name, experiences_offered,neighborhood_overview,notes,transit,access,interaction,house_rules,instant_bookable
```


```{r}
description_best <- data.frame(matrix(ncol = 3, nrow = 33))
colnames(description_best) <- c('neighbourhood', 'high_kpi_descript', 'low_kpi_descript')
j=1
for (i in 1:nrow(description_analysis)){
  description_analysis$descrip[[i]]=description_analysis$descrip[[i]][!description_analysis$descrip[[i]] %in% c("the","a","is","to","and","with","in","of","for","from","are","will","our","you","your","it","by","be","we","on","very","or","has","have","this","at","can","room")]
  if(description_analysis$kpi_score[[i]]=="high"){
    description_best$neighbourhood[j]=description_analysis$neighbourhood_cleansed[[i]]
    description_best$high_kpi_descript[j]=sort(table(description_analysis$descrip[[i]]),decreasing = TRUE)[1]
  }
  else{
    description_best$low_kpi_descript[j]=sort(table(description_analysis$descrip[[i]]),decreasing = TRUE)[1]
    j=j+1
  }
}
...
```{r}
description_best
unique(description_analysis$descrip[[1]])
#sort(table(description_analysis$descrip[[i]]),decreasing = TRUE)
```
...
# London map
ggplot()+geom_polygon(data=text_map, aes(x=x, y=y, group = name),colour="grey", fill="white") +
  geom_text(data=summary,aes(x=mean_long, y=mean_lat, label = high_kpi_descript ))
