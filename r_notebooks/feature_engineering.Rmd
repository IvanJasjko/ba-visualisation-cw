...
```{r}
requirements <- c("ggplot2", "dplyr", "stringr", "tidyr", "ggrepel", "splitstackshape")
...
### Use wid_data dataframe thati includes all amenities (Feel free to remove all cells below)
```{r}
head(wide_data)
```
```{r}
ggplot(data=wide_data) +
  geom_area(aes(x = kpi, y = ..count.., fill=as.character(a_smoke_detector)), stat = "bin",
            binwidth=0.015, color="darkblue", alpha=0.5)+
  labs(fill="a_smoke_detector")
```
